...
Let $I \in [0,255]^{H \times W \times 3}$ be the clean page in BGR order (channel index $c \in \{B,G,R\}$). All blending is done in floating point; the final image is clipped to $[0,255]$ and quantized to 8 bits.
...
$$
\texttt{seed} = \Big( \mathrm{SHA256}\big(\texttt{utf8}(\texttt{source\_id} \parallel p \parallel k)\big)_{0:8} \Big) \bmod 2^{31}
$$

interpreting the first eight digest bytes as a big-endian unsigned integer.
